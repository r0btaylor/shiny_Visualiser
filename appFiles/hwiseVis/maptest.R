...
server <- shinyServer(function(input, output, session) {
  ...
  observeEvent({
    ...
    if (class(raw.dataA()[[input$circle_color]])=="numeric") {
      ...
      # } else if (input$style == "Heatmap") {
      #   colorData <- filtered_data()[[input$circle_color]]
      #   color <- colorNumeric(pal1(length(levels(factor(raw.dataA()[[input$circle_color]])))), domain = raw.dataA()[[input$circle_color]])
      #   legend_vals = raw.dataA()[[input$circle_color]]
    ...
    if (input$style == "Points" & phase() == 1 ) {
      ...
      leafletProxy("mymap") %>%
        ...
        addHeatmap(filtered_data()[, "gps-longitude"],
                   filtered_data()[, "gps-latitude"],
                   group = "points", blur = 20, max = 0.05, radius = 15,
                   #gradient = rev(pal1(length(levels(factor(filtered_data()[, input$circle_color]))))),
                   intensity = filtered_data()[[input$circle_color]]
                   #max = max(filtered_data()[, input$circle_color]),
                   #cellSize = 1
        ) 

...
# Data -------------------------------------------------------------------------
  ...
  filter_tidy <- reactive({
    ...
    names <- unlist(lapply(input$varchoices1, function(x) varGroups1[[x]]))
...
# Plots tab --------------------------------------------------------------------
  ...
  output$bar_plot <- renderPlotly({
    ...
    g <- clean.data()[[input$variable_groupby_bar]]
  ...
  output$box_plot <- renderPlotly({
    ...
    g <- clean.data()[[input$variable_groupby_box]]
  ...
  output$scatter_plot <- renderPlotly({
    ...
    g <- clean.data()[[input$variable_groupby_scat]]
  ...
  output$var_toggle1 <- renderUI({
    radioButtons(
      "var_toggle1", "Select categorical variables to display:",
  ...
  output$var_toggle <- renderUI({
    radioButtons(
      "var_toggle", "Select numeric variables to display:",
  ...
  output$circle_color <- renderUI({
    `%!in%` <- Negate(`%in%`)
    choices = c(colnames(raw.dataA()[,colnames(raw.dataA()) %!in% c("village","gps-latitude","gps-longitude","sd11_1","w8","wat5a","wat8","wat14_3","wat16","wat17","liv5","liv6")]))

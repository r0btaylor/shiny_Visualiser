...
ui <- fluidPage(
  ...
  fluidRow(
    useSweetAlert(),
    column(2, div(id = "img-id", img(src = "hwise_logo.jpg"), style = {"padding-left:20px"})),
    column(2, div(id = "img-id", img(src = "gwr_logo.png"), style = {"padding-left:20px"})),
...
# Ribbon -----------------------------------------------------------------------
  ...
  questions_vec <- reactive ({
    ...
    # (DRY) ----
    if (phase() == 1) {
      # Match cols to clean.data()
      df <- df[, colnames(df) %in% colnames(clean.data())]
  ...
  raw.dataA <-  reactive({
    ...
    if (phase() == 1) {
      ...
      # Add newly created variables
      `%!in%` <- Negate(`%in%`)
      df <- bind_cols(df, clean.data()[, colnames(clean.data()) %!in% colnames(df)])
      df <- df %>%
        relocate(sd5_, .after = sd4) %>%
        relocate(sd6_, .after = sd5_) %>%
        relocate(wat12_, .before = wat12_1) %>%
      ...
      df$camp <- clean.data()$camp
      
      df$sd11_2 <- clean.data()$sd11_2
  ...
  filter_clean <- reactive({
      ...
      if (phase() == 1) {
        ...
        "Block" = "block",
        "Sex"="sex",
        "Sociodemographic"=c(colnames(clean.data()[, grepl("sd", names(clean.data()))])),
        "Work"=c("w1_1","w2","w3","w4","w5","w6","w7","w8"),
        "Perception of Water Services"=c("p1","p2","p3","p4","p5"),
        "HWISE"=c(colnames(clean.data()[, grepl("hw", names(clean.data()))])),
        "Water Access, Quantity, Utility, Stability"=c(colnames(clean.data()[, grepl("wat", names(clean.data()))])),
        "Livelihood Activities"=c(colnames(clean.data()[, grepl("liv", names(clean.data()))])),
        "Paying for Water"=c(colnames(clean.data()[, grepl("pay", names(clean.data()))])),
        "Well-being"=c(colnames(clean.data()[, grepl("wb", names(clean.data()))]))
      )
      ...
      # (WET) ----
        ...
        varGroups <- list(
          ...
          "Sex"="sex",
          "Sociodemographic"=c(colnames(clean.data()[, grepl("sd", names(clean.data()))])),
          "Livelihood & Income"=c(colnames(clean.data()[, grepl("li", names(clean.data()))])),
          "HWISE"=c(colnames(clean.data()[, grepl("hw", names(clean.data()))])),
          "Water Access, Quantity, Utility, Stability"=c(colnames(clean.data()[, grepl("wat", names(clean.data()))])),
          "Paying for Water"=c(colnames(clean.data()[, grepl("pay", names(clean.data()))])),
          "Well-being"=c(colnames(clean.data()[, grepl("wb", names(clean.data()))]))
    ...
    clean.data() %>% select(all_of(names))
  })
  ...
  filter_tidy <- reactive({
    ...
    if (phase() == 1) {
      varGroups <- list(
        ...
        "Block" = "block",
        "Sex"="sex",
        "Sociodemographic"=c(colnames(raw.dataA()[, grepl("sd", names(raw.dataA()))])),
        "Work"=c("w1_1","w2","w3","w4","w5","w6","w7","w8"),
        "Perception of Water Services"=c("p1","p2","p3","p4","p5"),
        "HWISE"=c(colnames(raw.dataA()[, grepl("hw", names(raw.dataA()))])),
        "Water Access, Quantity, Utility, Stability"=c(colnames(raw.dataA()[, grepl("wat", names(raw.dataA()))])),
        "Livelihood Activities"=c(colnames(raw.dataA()[, grepl("liv", names(raw.dataA()))])),
        "Paying for Water"=c(colnames(raw.dataA()[, grepl("pay", names(raw.dataA()))])),
        "Well-being"=c(colnames(raw.dataA()[, grepl("wb", names(raw.dataA()))]))
      )
    # (WET) ----
    } else if (phase() ==2) {
      varGroups <- list(
        ...
        "Sex"="sex",
        "Sociodemographic"=c(colnames(raw.dataA()[, grepl("sd", names(raw.dataA()))])),
        "Livelihood & Income"=c(colnames(raw.dataA()[, grepl("li", names(raw.dataA()))])),
        "HWISE"=c(colnames(raw.dataA()[, grepl("hw", names(raw.dataA()))])),
        "Water Access, Quantity, Utility, Stability"=c(colnames(raw.dataA()[, grepl("wat", names(raw.dataA()))])),
        "Paying for Water"=c(colnames(raw.dataA()[, grepl("pay", names(raw.dataA()))])),
        "Well-being"=c(colnames(raw.dataA()[, grepl("wb", names(raw.dataA()))]))
  ...
  output$clean.data <- renderDataTable({
    clean.data()
  })
  ...
  output$sum_plot <- renderPlot({
    ...
    x <- clean.data()[[input$group_stats]]
    ...
    p1<- ggplot(na.omit(temp),aes(factor(x))) +
      ...
      scale_x_discrete(breaks = x,
                       labels = raw.dataA()[[input$group_stats]]) +
  ...
  output$bar_plot <- renderPlotly({
    ...
    x <- clean.data()[[input$variable_x_bar]]
    g <- raw.dataA()[[input$variable_groupby_bar]]
    
    labelsx = raw.dataA()[[input$variable_x_bar]]
  ...
  output$box_plot <- renderPlotly({
    ...
    paletteFunc <- colorRampPalette(c("#18db50","#1d6cac","#00b4ff"))
    plot_cols <- paletteFunc(length(levels(factor(clean.data()[[input$variable_groupby_box]]))))
    
    x <- clean.data()[[input$variable_x_box]]
    y <- clean.data()[[input$variable_y_box]]
    g <- raw.dataA()[[input$variable_groupby_box]]
    labelsx = raw.dataA()[[input$variable_x_box]]
    labelsy = raw.dataA()[[input$variable_y_box]]
    ...
    p1 <- 
      ...
      if (input$variable_groupby_box != "None" &
          class(raw.dataA()[[input$variable_y_box]])=="character") {
        base_plot_p1 +
          scale_x_discrete(breaks = x, labels = labelsx) +
        ...
        # numeric y 
                 ...
                 class(raw.dataA()[[input$variable_y_box]])=="numeric") {
        base_plot_p1 +
          scale_x_discrete(breaks = x, labels = labelsx) +
  ...
  output$scatter_plot <- renderPlotly({
    ...
    paletteFunc <- colorRampPalette(c("#18db50","#1d6cac","#00b4ff"))
    plot_cols <- paletteFunc(length(unique(clean.data()[[input$variable_groupby_scat]])))
    
    x <- clean.data()[[input$variable_x_scat]]
    y <- clean.data()[[input$variable_y_scat]]
    g <- raw.dataA()[[input$variable_groupby_scat]]
    labelsx = raw.dataA()[[input$variable_x_scat]]
    labelsy = raw.dataA()[[input$variable_y_scat]]
    temp <- data.frame(x, y)
    ...
    p1<-
      ...
      if (class(raw.dataA()[[input$variable_x_scat]])=="character" &
          class(raw.dataA()[[input$variable_y_scat]])=="character") {
        base_plot_p1 +
          scale_x_continuous(breaks = x, labels = labelsx) +
          scale_y_continuous(breaks = y, labels = labelsy)
        # numeric x, numeric y
      } else if (class(raw.dataA()[[input$variable_x_scat]])=="numeric" &
                 class(raw.dataA()[[input$variable_y_scat]])=="numeric") {
        base_plot_p1 +
      ...
      } else if (class(raw.dataA()[[input$variable_x_scat]])=="character" &
                 class(raw.dataA()[[input$variable_y_scat]])=="numeric") {
        base_plot_p1 +
          scale_x_continuous(breaks = x, labels = labelsx) +
          scale_y_continuous(breaks = seq(0,max(y,na.rm=T),ceiling(max(y,na.rm=T)/10)))
  ...
  output$circle_color <- renderUI({
    ...
    `%!in%` <- Negate(`%in%`)
    if (phase() == 1) {
      choices = c(colnames(raw.dataA()[,colnames(raw.dataA()) %!in% c("village","gps-latitude","gps-longitude","sd11_1","w8",
                                                                      "wat5a","wat8","wat14_3","wat16","wat17","liv5","liv6")]))

    } else if (phase() == 2 ) {
      choices = c(colnames(raw.dataA()[,colnames(raw.dataA()) %!in% c("village","gps-latitude","gps-longitude","li8","li8_1",
  ...
  observeEvent(input$style, {
    if (input$style == "Heatmap") {
      choices = raw.dataA() %>% 
        select(where(is.numeric)) %>% 
        select(-c("gps-latitude","gps-longitude"))%>%
        colnames
      updatePickerInput(session, "circle_color", choices = choices,
                        selected = "hw",
                        choicesOpt = list(
                          subtext = paste(questions_vec()[1, choices])
                        ))
  ...
  output$points_select <- renderUI({
    tagList(
      checkboxGroupInput("quality_toggle", "Water quality problems experienced?",
                         choices  = c(levels(factor(raw.dataA()$wat7))),
                         selected = c(levels(factor(raw.dataA()$wat7)))
      ),
      checkboxGroupInput("source_toggle", "Primary drinking water source:",
                         choices  = c(levels(factor(raw.dataA()$wat1))),
                         selected = NULL
      ),
      checkboxGroupInput("hw2_toggle", "Water supply interruptions:",
                         choices  = c(levels(factor(raw.dataA()$hw2))),
                         selected = NULL
      ),
      checkboxGroupInput("ward_toggle", "Ward:",
                         choices  = c(levels(factor(raw.dataA()$ward))),
                         selected = NULL
      )
    )
  })
  ...
  filtered_data <- eventReactive({
    ...
    input$ward_toggle}, {
      ...
      if(is.null(input$source_toggle) & is.null(input$hw2_toggle) & is.null(input$quality_toggle) & is.null(input$ward_toggle)){
        results <- raw.dataA() %>% filter(FALSE)
      }
      return(results)
    }, ignoreNULL = FALSE)
  ...
  # Read shape files
  camps <- st_read(dsn = "shapefiles/camp", layer = "T220130_RRC_Outline_Camp_AL1")
  camps <- subset(camps, camps$Upazila=="Teknaf")
  
  unions <- st_read(dsn = "shapefiles/union", layer = "bgd_admbnda_adm4_bbs_20201113")
  unions <- subset(unions, unions$ADM4_EN %in% c("Baharchhara","Nhilla","Sabrang","Teknaf","Teknaf Paurashava","Whykong"))
  
  # read infrastructure data
  infra <- st_read(dsn = "shapefiles/infra", layer = "WASH_Infras_LT_Bath_TW_May_31_2022")
  ...
  output$mymap <- renderLeaflet({
    ...
    leaflet(data = raw.dataA()) %>% 
      setView(lng = raw.dataA()[1, "gps-longitude"],
              lat = raw.dataA()[1, "gps-latitude"],
              zoom=12) %>%
  ...
  observeEvent({
    ...
    if (class(raw.dataA()[[input$circle_color]])=="numeric") {
      colorData <- filtered_data()[[input$circle_color]]
      color <- colorNumeric(rev(pal1(9)), domain = raw.dataA()[[input$circle_color]])
      legend_vals = raw.dataA()[[input$circle_color]]
    ...
    if (input$style == "Points" & phase() == 1 ) {
      ...
      leafletProxy("mymap") %>%
        ...
        removeControl("colorLegend") %>%
        addHeatmap(filtered_data()[, "gps-longitude"],
                   filtered_data()[, "gps-latitude"],
                   group = "points",
                   intensity = filtered_data()[, input$circle_color],
                   max = max(filtered_data()[, input$circle_color]),
                   cellSize = 1
                   ) %>%
        addLegend("topright",
                  pal=color,
                  ...
                  layerId="colorLegend")
    # (DRY) ----
    } else if (input$style == "Points" & phase() == 2) {
      leafletProxy("mymap") %>%
        clearGroup("points") %>%
        addProviderTiles("CartoDB.Positron", group = "Minimal Map") %>%
        addProviderTiles("Esri.WorldImagery", group = "Satelite Map") %>%
  ...
  filtered_Qdata <- reactive({
    ...
    df <- data.frame(t(df)) %>% 
      rename("Question" = "X2")
    df$`Example Responses` = apply(data.frame(raw.dataA()[, colnames(raw.dataA()) %in% rownames(df)]), 
                                   2, 
